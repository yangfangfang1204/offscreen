...
int g_size=g_nFrameWidth*g_nFrameHeight*3/2;
const char* g_pHwd1=NULL;
const char* g_pHwd2=NULL;
...
void initContextGL()
{
   ...
   // attr.proxyContextToMainThread = EMSCRIPTEN_WEBGL_CONTEXT_PROXY_FALLBACK;//EMSCRIPTEN_WEBGL_CONTEXT_PROXY_FALLBACK
    ...
    EMSCRIPTEN_WEBGL_CONTEXT_HANDLE glContext = emscripten_webgl_create_context(g_pHwd1, &attr);
    assert(glContext);
    //emscripten_set_canvas_element_size(g_pHwd1, 355, 233);
...
#define EXPLICIT_SWAP
  ...
  REPORT_RESULT(0);
#endif
   //emscripten_request_animation_frame(RenderFrame,(void*)1);
   glUseProgram(NULL);
   return 1;
}
...
EMSCRIPTEN_KEEPALIVE
int InitMain(int nFrameWidth,int nFrameHeight,int size,void* hwnd1,void* hwnd2) {
    g_pHwd1=(char*)hwnd1;
	g_pHwd2=(char*)hwnd2;
    ...
    g_yuvLen=size;
    g_size=g_yuvLen;
    //printf("InitMain: nFrameWidth:%d,nFrameHeight:%d,size:%d,g_pHwd1:%s,g_pHwd2:%s\n",nFrameWidth,nFrameHeight,size,g_pHwd1,g_pHwd2);
    //get_canvas_size();
    ...
    pthread_create(&thread, &attr, displayThread, NULL);
